...
<?php if (!$rentals): ?>
  <div class="bg-white border rounded-xl p-6 text-gray-600">Ainda não alugaste nenhum livro.</div>
<?php else: ?>
  <div class="space-y-4">
      ...
      <div class="bg-white border rounded-xl p-5">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="text-sm text-gray-600"><?= htmlspecialchars($r['author']) ?></p>
            <p class="text-sm text-gray-500 mt-1"><?= htmlspecialchars($r['category']) ?></p>
          </div>
        ...
        <div class="mt-4 text-sm text-gray-700">
          <p><strong>Data de aluguer:</strong> <?= htmlspecialchars($r['rented_at']) ?></p>
          <p><strong>Prazo de entrega:</strong> <?= htmlspecialchars($r['due_at']) ?></p>
          <p><strong>Renovações:</strong> <?= (int)$r['renewed_count'] ?>/2</p>
          <?php if ($r['returned_at']): ?>
            <p><strong>Devolvido em:</strong> <?= htmlspecialchars($r['returned_at']) ?></p>
          <?php endif; ?>
        </div>
          ...
          <div class="mt-4 flex gap-2">
            <?php if ((int)$r['renewed_count'] < 2): ?>
              <form method="post" action="/bookwave/public/renew_book.php">
                ...
                <button type="submit" class="px-4 py-2 rounded bg-blue-600 text-white">Renovar</button>
              </form>
            <?php endif; ?>

            <form method="post" action="/bookwave/public/return_book.php">
              <input type="hidden" name="rental_id" value="<?= (int)$r['id'] ?>">
              <button type="submit" class="px-4 py-2 rounded bg-red-600 text-white">Devolver</button>
            </form>
          </div>
        <?php endif; ?>
      </div>
