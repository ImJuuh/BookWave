      ...
      if ($existingUser) {
        ...
        if ($newPassword !== '' || $confirmPassword !== '') {
          if (strlen($newPassword) < 6) {
            ...
            $stmt->execute([$name, $email, password_hash($newPassword, PASSWORD_DEFAULT), $user['id']]);
...
<div class="max-w-xl mx-auto bg-white border rounded-xl p-6">
  ...
  <form method="post">
    ...
    <input type="text" name="name" value="<?= htmlspecialchars($user['name']) ?>" class="w-full border rounded px-3 py-2 mb-4" required>

    <label class="block mb-1 font-medium">Email</label>
    <input type="email" name="email" value="<?= htmlspecialchars($user['email']) ?>" class="w-full border rounded px-3 py-2 mb-4" required>
    ...
    <input type="password" name="new_password" class="w-full border rounded px-3 py-2 mb-4">

    <label class="block mb-1 font-medium">Confirmar nova password</label>
    <input type="password" name="confirm_password" class="w-full border rounded px-3 py-2 mb-6">

    <button type="submit" class="w-full bg-gray-900 text-white rounded px-4 py-2">Guardar alterações</button>
  </form>
</div>
